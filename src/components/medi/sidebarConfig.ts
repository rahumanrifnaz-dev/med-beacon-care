...
// Centralized menu config for all roles. Keep URLs stable here.
import {
  LayoutDashboard,
  MessageCircle,
  Pill,
...
export const MENU_ITEMS: NavItemConfig[] = [
  ...
  // Shared message/notification/settings links are included for relevant roles
...
export function getRoleNav(role: AppRole) {
  // Convert config items to the shape used by DashboardShell (icon may be optional)
  return MENU_ITEMS.filter((i) => i.roles.includes(role)).map((i) => ({ label: i.label, to: i.to, icon: i.icon }));
}
